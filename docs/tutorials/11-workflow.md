# A complete workspace workflow

Combine the earlier concepts into one small report job. The same full program
runs with Bash, PowerShell, or cmd and needs no external service or project data.

1. Create isolated input and log directories; configure full capture.
2. Navigate into the project and export the fruit to count.
3. Select a native script that verifies the input file exists. `check=True`
   prevents later steps from continuing after a failed precondition.
4. Run a structural Python command that inherits cwd and environment, processes
   the file, and writes a JSON report to stdout.
5. Decode the result, verify the full capture file, and close the shell before
   removing the workspace. The assertions document why the report counts two
   apples rather than all three lines.

In an application, replace the temporary workspace with your explicitly owned
project/log directories, keep the same ownership scopes, and handle the typed
failures described earlier. The example prints the report and intentionally
removes its demonstration files after verifying them.

## Full example

File: [`examples/11-workflow.py`](https://github.com/gokurakujoudo/shell-next/blob/main/examples/11-workflow.py).

```console
python examples/11-workflow.py bash
```

On Windows replace `bash` with `powershell` or `cmd`.

<!-- python-doc-exec native: 11-workflow.py -->
```python
